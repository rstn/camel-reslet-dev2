...
import com.peterservice.camel.component.restlet.RestletEndpoint;
...
public class RestletConsumer extends DefaultConsumer {
    private static final Logger currentLog = LoggerFactory.getLogger(RestletConsumer.class);
    private LocalRestlet restlet;

    private class LocalRestlet extends Restlet {       
        private void processExchange(Exchange exchange) throws Exception {
            try {
                getProcessor().process(exchange);
            } catch (Exception e) {
                currentLog.debug(e.getMessage(), e);
                exchange.setException(e); throw e;
            }
        }        
        @Override
        public void handle(final Request request, final Response response) {
            if (currentLog.isDebugEnabled()){
                currentLog.debug("Consumer restlet handle request method: {}", request.getMethod());
            }            
            try {
                final Exchange exchange = getEndpoint().createExchange();                
                final RestletBinding binding = getEndpoint().getRestletBinding();
